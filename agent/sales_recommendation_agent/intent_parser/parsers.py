...
class HeuristicDemandParser:
    ...
    def parse(self, raw_text: str) -> CustomerDemand:
        ...
        demand = CustomerDemand(
            ...
            server_or_idc_required=infer_positive_or_negative(text, positive=("服务器", "IDC", "机房", "云主机", "托管", "官网", "业务接口"), negative=()),
...
def extract_fixed_ip_count_text(text: str) -> str | None:
    ...
    match = re.search(r"(\d+)\s*(?:个)?\s*(?:固定\s*)?(?:公网\s*)?(?:IP|地址)", text, re.I)
...
def infer_primary_goal(text: str, category_candidate_keywords: list[str]) -> str | None:
    ...
    if contains_any(text, ("官网", "业务接口", "服务器对外", "对外访问")):
        return "服务器对外访问"
...
def infer_access_source(text: str) -> str | None:
    ...
    if "门店" in text:
        return "门店"
    if contains_any(text, ("官网", "业务接口", "服务器对外", "固定公网IP", "公网IP")):
        return "服务器/官网对外访问"
    return None
