...
def build_readiness_rules() -> dict[str, ReadinessRule]:
    ...
    return {
        ...
        "上网-固定IP上网": ReadinessRule(
            ...
            assumable={
                "fixed_ip_count": "客户未明确固定公网 IP 数量，暂按 13 个以内优先精品专线、超过 13 个再核实 IPMAN 方向处理。",
            ...
            clarification_intents={
                ...
                "fixed_ip_count": ClarificationIntent(
                    field="fixed_ip_count",
                    priority="high",
                    intent="确认固定公网 IP 数量",
                    why="固定 IP 数量会直接影响候选方向：13 个以内通常优先核实精品专线，超过 13 个或大段 IP 通常转向 IPMAN。",
                    example_options=["1个", "2-13个", "超过13个", "客户暂不确定"],
                    term_explanations=[fixed_ip_explanation],
                ),
...
def build_human_fallback_question(intent: ClarificationIntent) -> str:
    ...
    if intent.field == "fixed_ip_required":
        return "官网、服务器或固定出口是否需要长期使用同一个公网地址？"
    if intent.field == "fixed_ip_count":
        return "大概需要几个固定公网 IP？"
