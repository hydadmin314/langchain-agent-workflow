(function () {
  const dataset = normalizeDataset(window.REVIEW_DASHBOARD_DATA);
  const state = {
    query: "",
    family: "all",
    carrier: "all",
  ...
  function normalizeDataset(input) {
    ...
    const documents = Array.isArray(source.documents) ? source.documents.map(normalizeDocument) : [];
    return {
      generatedAt: source.generatedAt || "",
      sourceDir: source.sourceDir || "",
      documents,
      packages: Array.isArray(source.packages) ? source.packages : [],
      families: Array.isArray(source.families) ? source.families : unique(documents.map((doc) => doc.productFamily)),
      carriers: Array.isArray(source.carriers) ? source.carriers : unique(documents.map((doc) => doc.carrier)),
    };
  }
  ...
  function normalizeDocument(doc) {
    return {
      ...doc,
      basePackages: Array.isArray(doc.basePackages) ? doc.basePackages : [],
      optionalPackages: Array.isArray(doc.optionalPackages) ? doc.optionalPackages : [],
      materials: Array.isArray(doc.materials) ? doc.materials : [],
      feeRules: Array.isArray(doc.feeRules) ? doc.feeRules : [],
      agreementRules: Array.isArray(doc.agreementRules) ? doc.agreementRules : [],
      constraints: Array.isArray(doc.constraints) ? doc.constraints : [],
      supplementalRules: Array.isArray(doc.supplementalRules) ? doc.supplementalRules : [],
      validationIssues: Array.isArray(doc.validationIssues) ? doc.validationIssues : [],
      schemaWarnings: Array.isArray(doc.schemaWarnings) ? doc.schemaWarnings : [],
      serviceAttributes: Array.isArray(doc.serviceAttributes) ? doc.serviceAttributes : [],
      includedItems: Array.isArray(doc.includedItems) ? doc.includedItems : [],
      priceSummary: doc.priceSummary || { count: 0, min: null, max: null, periods: [] },
      qualityScore: Number.isFinite(doc.qualityScore) ? doc.qualityScore : 0,
      qualityLabel: doc.qualityLabel || "需复核",
      issueCount: Number.isFinite(doc.issueCount) ? doc.issueCount : 0,
      warningCount: Number.isFinite(doc.warningCount) ? doc.warningCount : 0,
    };
  }
  ...
  function summarize() {
    ...
    return [
      ...
      ["可报价套餐", `${dataset.packages.length}`, "已抽取到价格或档位"],
      ["办理材料", `${materialCount}`, "成交后可直接提醒"],
      ["可直接推荐", `${readyDocs}`, "信息较完整的候选"],
      ["待人工复核", `${needsReview}`, "需要回看来源原文"],
      ["最高报价", priced.length ? money.format(Math.max(...priced.map((item) => item.price))) : "待确认", "按已抽取价格统计"],
    ];
  }
    ...
    docs.forEach((doc) => {
      ...
      button.append(
        ...
        create("div", { className: "doc-meta" }, [
          ...
          doc.issueCount ? badge(`${doc.issueCount} 项待查`, "bad") : badge("校验正常", "good"),
        ]),
  ...
  function renderDetail(doc) {
    if (!doc) {
      ...
      $("detailTitle").textContent = "暂无可展示文档";
      $("detailMeta").textContent = "请先生成 data.js，或检查 published JSON 目录。";
      $("detailScore").textContent = "0%";
      $("detailScoreBar").style.width = "0%";
      $("insightGrid").replaceChildren();
      $("tabPanel").replaceChildren(empty("没有可展示的数据。"));
      return;
    }
    $("detailBadges").replaceChildren(
      badge(doc.qualityLabel, qualityClass(doc.qualityLabel)),
    ...
    $("insightGrid").replaceChildren(
      ...
      insight("规则与风险", `${doc.feeRules.length + doc.agreementRules.length + doc.constraints.length} 条`, doc.issueCount ? `${doc.issueCount} 项待复核` : "校验通过")
    );
    renderTab(doc);
  }
  ...
  function salesNotes(doc) {
    const notes = [];
    if (doc.basePackages.length) notes.push(`可以进入报价推荐：当前有 ${doc.basePackages.length} 个基础套餐档位。`);
    else notes.push("暂不适合直接报价：缺少基础套餐档位，建议先补齐价格和带宽。");
    if (doc.priceSummary.count) notes.push(`价格区间为 ${priceRange(doc.priceSummary)}，可用于预算匹配和方案对比。`);
    else notes.push("还没有明确价格，销售话术里需要提示客户以人工确认为准。");
    if (doc.materials.length) notes.push(`已整理 ${doc.materials.length} 项办理材料，可作为成交后的交付提醒。`);
    if (doc.constraints.some((item) => item.blocks_recommendation)) notes.push("存在阻断类约束，推荐前要先确认开通条件。");
    if (doc.issueCount || doc.warningCount) notes.push("存在抽取或 schema 提醒，对外展示前建议回看来源原文。");
    return notes;
  }
  ...
  function renderOverview(doc) {
    ...
    return create("div", { className: "overview-grid" }, [
      ...
      create("section", { className: "plain-section wide" }, [
        create("h3", { text: "下一步建议" }),
        create("div", { className: "action-grid" }, [
          actionTile("确认人数与带宽", "用于筛选套餐档位和预算上限。"),
          actionTile("确认地址资源", "判断本地开通条件、跨域接入或固定 IP。"),
  ...
  async function writeClipboard(text) {
    ...
    const textarea = create("textarea", { className: "clipboard-fallback" });
    textarea.value = text;
  ...
  async function copySource() {
    ...
    if (!doc) return;
    try {
      await writeClipboard(doc.reviewFile || doc.sourcePath);
      toast("已复制 JSON 路径");
    } catch (error) {
      toast("复制失败，请检查浏览器权限");
    }
